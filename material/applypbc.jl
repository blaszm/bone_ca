# functions to apply periodic boundary conditions to the tangent matrix and residual and undo the operation after solving the linear system

# returns a helper array: nodedoflist(i, :) = [nodenumber, fielddof], where i = dof in dh 
function nodedofs(dh::DofHandler, grid::Grid) 
    ...
    @inbounds for (i,_) in enumerate(grid.nodes)
...
# calculates reduced tangent matrix K_red from full system K and BoundaryData bdata
function applyPBC_S(ctx::CoherentStructures.GridContext, bdata::CoherentStructures.BoundaryData, S::SparseMatrixCSC, nodedoflist::Array{Int64,2}, ndof::Int) # gridcontext/boundary data/stiffness matrix/list of nodenumber and coressponding DoFs/dof per nodes
    K = deepcopy(S)
    droptol!(K, 1e-20)
    _, m = size(K) # assume quadratic matrix
...
# calculates reduced residual r_red from full vector n and BoundaryData bdata
...
# reverses appplication of PBC for reduced solution vector ΔΔu_red to calculate full vector ΔΔu using BoundaryData bdata
